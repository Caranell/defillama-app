...
describe('dataset cache registry', () => {
	...
	it('uses registry trace helpers for configured standalone routes', () => {
		...
		const expectedRouteDomains: Record<string, DatasetDomain[]> = {
			...
			'/yields/pool/*': ['yields'],
			'/api/public/yields/borrow': ['yields'],
			'/api/public/yields/borrow-advanced': ['yields'],
			'/api/public/yields': ['yields'],
			'/api/public/yields/pools': ['yields'],
			'/api/public/yields/token-borrow-routes': ['yields'],

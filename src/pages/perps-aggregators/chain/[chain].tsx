...
export const getStaticProps = withPerformanceLogging(
	...
	async ({ params }: GetStaticPropsContext<{ chain: string }>) => {
		...
		const chainRoute = resolveChainFeatureParamFromMetadata(params.chain, metadataCache, (metadata) =>
			Boolean(metadata.perpsAggregators)
		)
		...
		addDimensionChainRouteTelemetry({
			...
			canonicalRoute: '/perps-aggregators/chain/[chain]',
			dataType,
			metadataFlag: 'perpsAggregators'
		})

		if (!chainRoute) {
			return { notFound: true }
		}
		const redirectDestination = getChainRouteRedirectDestination(params.chain, chainRoute, 'perps-aggregators/chain')
		...
		if (!data) throw new Error(`Missing page data for route=/perps-aggregators/chain/[chain] chain=${chain}`)

		const { questions: entityQuestions } = await fetchEntityQuestions(chainRoute.canonicalSlug, 'chain', {
			subPage: 'perps-aggregators',

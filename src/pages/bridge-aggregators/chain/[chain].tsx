...
const dataType = ADAPTER_DATA_TYPES.DAILY_BRIDGE_VOLUME
...
export const getStaticProps = withPerformanceLogging(
	...
	async ({ params }: GetStaticPropsContext<{ chain: string }>) => {
		...
		const chainRoute = resolveChainFeatureParamFromMetadata(params.chain, metadataCache, (metadata) =>
			Boolean(metadata.bridgeAggregators)
		)
		...
		addDimensionChainRouteTelemetry({
			...
			canonicalRoute: '/bridge-aggregators/chain/[chain]',
			dataType,
			metadataFlag: 'bridgeAggregators'
		})

		if (!chainRoute) {
			return { notFound: true }
		}
		const redirectDestination = getChainRouteRedirectDestination(params.chain, chainRoute, 'bridge-aggregators/chain')
		...
		if (!data) throw new Error(`Missing page data for route=/bridge-aggregators/chain/[chain] chain=${chain}`)

		const { questions: entityQuestions } = await fetchEntityQuestions(chainRoute.canonicalSlug, 'chain', {
			subPage: 'bridge-aggregators',

...
export const getStaticProps = withPerformanceLogging(
	...
	async ({ params }: GetStaticPropsContext<{ chain: string }>) => {
		...
		addDimensionChainRouteTelemetry({
			...
			canonicalRoute: '/holders-revenue/chain/[chain]',
			dataType,
			metadataFlag: 'fees'
		})
		...
		const redirectDestination = getChainRouteRedirectDestination(params.chain, chainRoute, 'holders-revenue/chain')
		...
		if (!data) throw new Error(`Missing page data for route=/holders-revenue/chain/[chain] chain=${chain}`)
		...
		const { questions: entityQuestions } = await fetchEntityQuestions(chainRoute.canonicalSlug, 'chain', {
			...
			topProtocols: data.protocols.slice(0, 15).map((p) => ({
				name: p.name,
				holdersRevenue24h: p.total24h ?? null,
				holdersRevenue7d: p.total7d ?? null,
				mcap: p.mcap ?? null,

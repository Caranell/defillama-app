...
export const getStaticProps = withPerformanceLogging(
	...
	async ({ params }: GetStaticPropsContext<{ protocol: string }>) => {
		...
		const protocolRoute = resolveProtocolFeatureRouteFromMetadata({
			hasMetric: (metadata) => Boolean(metadata.perps),
			metadataCache,
			protocol,
			routePrefix: 'protocol/perps'
		})
		if (!protocolRoute) {
			return { notFound: true }
		}
		...
		const perpVolume: IProtocolOverviewPageData['perpVolume'] = formatAdapterData({
			data: adapterData,
		...
		return {
			props: {
				...
				hallmarks,
				defaultChartView: perpVolume?.defaultChartView ?? 'daily',
				seoTitle,
				seoDescription
			},
...
export default function Protocols(props: InferGetStaticPropsType<typeof getStaticProps>) {
	...
	const finalCharts = useMemo(() => {
		...
		return {
			dataset: {
				source: formattedData.map(([timestamp, value]) => ({ timestamp, 'Perp Volume': value })),
				dimensions: ['timestamp', 'Perp Volume']
			},
			charts: [
				{
					type: (groupBy === 'cumulative' ? 'line' : 'bar') as 'line' | 'bar',
					name: 'Perp Volume',
					encode: { x: 'timestamp', y: 'Perp Volume' },

...
export const getStaticProps = withPerformanceLogging(
	...
	async ({ params }: GetStaticPropsContext<{ chain: string }>) => {
		...
		addDimensionChainRouteTelemetry({
			...
			canonicalRoute: '/earnings/chain/[chain]',
			dataType,
			metadataFlag: 'fees'
		})
		...
		const redirectDestination = getChainRouteRedirectDestination(params.chain, chainRoute, 'earnings/chain')
		...
		if (!data) throw new Error(`Missing page data for route=/earnings/chain/[chain] chain=${chain}`)
...
const EarningsOnChain = (props: IAdapterByChainPageData) => {
	return (
		<Layout
			...
			canonicalUrl={`/earnings/chain/${slug(props.chain)}`}
